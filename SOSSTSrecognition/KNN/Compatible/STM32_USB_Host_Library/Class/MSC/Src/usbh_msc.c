...
uint32_t _debug_msc_interface_init_stage;
MSC_HandleTypeDef _msc_handle;
static USBH_StatusTypeDef USBH_MSC_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  {
    _debug_msc_interface_init_stage = 1;
  ...
  if (status != USBH_OK)
  {
    _debug_msc_interface_init_stage = 2;
    return USBH_FAIL;
  }

  //memset(&_msc_handle, 0, sizeof(MSC_HandleTypeDef));

  //phost->pActiveClass->pData = &_msc_handle;//(MSC_HandleTypeDef *)USBH_malloc(sizeof(MSC_HandleTypeDef));
  phost->pActiveClass->pData = (MSC_HandleTypeDef *)USBH_malloc(sizeof(MSC_HandleTypeDef));
  MSC_Handle = (MSC_HandleTypeDef *) phost->pActiveClass->pData;

  if (MSC_Handle == NULL)
  {
    _debug_msc_interface_init_stage = 3;
  ...
  {
    _debug_msc_interface_init_stage = 4;
  ...
  {
    _debug_msc_interface_init_stage = 5;
    return USBH_NOT_SUPPORTED;
  }
        ...
        switch (MSC_Handle->unit[MSC_Handle->current_lun].state)
        {
          ...
          case MSC_READ_INQUIRY:
            ...
            else
            {
              ...
              {
                MSC_Handle->unit[MSC_Handle->current_lun].state = MSC_IDLE;//MSC_UNRECOVERED_ERROR;//jinu
          ...
          case MSC_READ_CAPACITY10:
            ...
            else
            {
              if (scsi_status == USBH_UNRECOVERED_ERROR)
              {
                MSC_Handle->unit[MSC_Handle->current_lun].state = MSC_IDLE;//MSC_UNRECOVERED_ERROR;
                MSC_Handle->unit[MSC_Handle->current_lun].error = MSC_ERROR;
              }
            }
            break;
...
/* USER CODE BEGIN USBH_MSC_GetState */
/**
  * @brief  USBH_MSC_GetState
  * This function returns the MSC state
  * @param  phost: Host handle
  * @retval MSC state
  */
MSC_StateTypeDef USBH_MSC_GetState(USBH_HandleTypeDef *phost)
{
  // MSC 핸들이나 클래스가 유효하지 않으면 IDLE(대기) 상태를 반환하여
  // 무한 루프에 빠지지 않도록 예외 처리합니다.
  if(phost->gState != HOST_CLASS || phost->pActiveClass == NULL || phost->pActiveClass->pData == NULL)
  {
    return MSC_IDLE; 
  }

  MSC_HandleTypeDef *MSC_Handle = (MSC_HandleTypeDef *) phost->pActiveClass->pData;
  return MSC_Handle->state;
}
/* USER CODE END USBH_MSC_GetState */
