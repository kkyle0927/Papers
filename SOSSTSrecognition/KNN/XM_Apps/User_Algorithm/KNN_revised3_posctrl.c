...
static const FVecModeParam_t kFVecModeParam[F_VECTOR_NUM_MODES] = {
    ...
    {0.006761846f, 0.013523692f, 0.006761846f, 1.980149875f, -0.980248813f,
     1.0f},
    {0.002257699f, 0.004515399f, 0.002257699f, 1.993344426f, -0.993355500f,
     1.0f},
    {0.001356427f, 0.002712852f, 0.001356427f, 1.996003996f, -0.996007988f,
     1.0f},
    {0.000904852f, 0.001809705f, 0.000904852f, 1.997335110f, -0.997336885f,
     1.0f},
    {0.000678891f, 0.001357783f, 0.000678891f, 1.998001000f, -0.998001999f,
     1.0f},
    {0.000339643f, 0.000679288f, 0.000339643f, 1.999000250f, -0.999000500f,
     1.0f},
    {0.000226762f, 0.000453524f, 0.000226762f, 1.999333444f, -0.999333556f,
     1.0f},
    {0.000170083f, 0.000340167f, 0.000170083f, 1.999500062f, -0.999500125f,
     1.0f},
    {0.000135891f, 0.000271783f, 0.000135891f, 1.999600040f, -0.999600080f,
     1.0f},
};
...
static float s_tau_out_R_old2 = 0.0f;
static float s_tau_out_L_old1 = 0.0f;
static float s_tau_out_L_old2 = 0.0f;
...
// --- ANGLE-BASED TRAPEZOIDAL ASSIST ---
// Profile: 0 → tau_max in TRAP_RISE_MS, hold until thigh >= target angle,
//          then tau_max → 0 in TRAP_FALL_MS.
...
static float s_assist_tau_max_R = 0.0f;
static float s_assist_tau_max_L = 0.0f;
static float s_assist_elapsed_R_ms = 0.0f;
static float s_assist_elapsed_L_ms = 0.0f;
static float s_target_angle_R = 40.0f;
static float s_target_angle_L = 40.0f;
static AssistPhase_t s_assist_phase_R = PHASE_DONE;
static AssistPhase_t s_assist_phase_L = PHASE_DONE;
static float s_fall_elapsed_R_ms = 0.0f;
static float s_fall_elapsed_L_ms = 0.0f;

// Real-time Swing Phase (%) for telemetry
...
// threshold = 0.8 * (swing-leg angle at that HC). Initial value requested: 10
// deg.
...
// Updated when an HC is classified as STS: threshold = 0.6 * (swing-leg angle
// at that HC). Initial value requested: 10 deg.
...
// CDC/Live-expression mirrors (s_* are static above; these are global and
// volatile).
...
// Debug-visible computed torque commands (orders) even when not actuating (e.g.
// STANDBY)
...
static float ComputeTrapezoidalTorque(float t_ms, float t_period_ms,
                                      float tau_max);
 ...
 * - XM_SetControlMode함수를 Setup에서 실시간 제어 모드 / 모니터링 모드를
 * 선택해야 합니다. (기본 모니터링 모드)
 ...
 * ================================================================= */
void User_Loop(void) {
  // CM 연결 상태를 최우선으로 확인하여, 연결이 끊겼을 경우 OFF 상태로 강제
  // 전환합니다.
  if (!XM_IsCmConnected()) {
...
static void Standby_Loop(void) {
  ...
  const bool recording_started =
      (s_prevRecordingState != Record_ON && Recording == Record_ON);
  s_prevRecordingState = Recording;
  ...
  if (XM_button1 == XM_BTN_CLICK) {
    if (tau_max_setting >= 7)
      tau_max_setting = 0;
    else
      tau_max_setting += 1;
  }

  // Handle Button 3: Short click for Adaptive toggle, Long press for Reset
  XM_button3 = XM_GetButtonEvent(XM_BTN_3);
  if (XM_button3 == XM_BTN_CLICK) {
    s_adaptive_assist_enabled = !s_adaptive_assist_enabled;
    // Feedback: could use LED or CDC here
  }
  ...
  if (XM_GetButtonState(XM_BTN_3) == XM_PRESSED) {
    ...
    if (s_btn3_press_timer >= 500) { // 1 second @ 500Hz
      ...
      Recording = recording_saved;
      s_prevRecordingState = Recording;
      s_btn3_press_timer = 0; // Reset timer after reset
    }
  } else {
    s_btn3_press_timer = 0;
  }

  // In STANDBY: compute assist "orders" (triggers + f-vector decode), but do
  // NOT actuate.
  trig_R = false;
  trig_L = false;
  ...
  adaptive_assist((const GaitRecognitionResult_t *)&s_gait_rec, (bool *)&trig_R,
                  (bool *)&trig_L, &ignored_R, &ignored_L);

  if (trig_R && !Rstop_assist) {
    float fvec[4] = {kManuscriptFVec[0], (float)tau_max_setting,
                     kManuscriptFVec[2], kManuscriptFVec[3]};
  ...
  // --- Angle-based trapezoidal torque (Standby: compute but don't actuate) ---
...
static void Active_Loop(void) {
  ...
  // --- USER_DEFINED_CTRL: trigger manuscript-defined f-vector and
  // decode/actuate in real-time ---
  trig_R = false;
  trig_L = false;
  float ignored_R = 0.0f, ignored_L = 0.0f;
  ...
  // FVec trigger (legacy, kept for compatibility but not primary torque source)
  if (trig_R && !Rstop_assist) {
    float fvec[4] = {kManuscriptFVec[0], (float)tau_max_setting,
                     kManuscriptFVec[2], kManuscriptFVec[3]};
    TriggerManuscriptFVecSingle(s_fvec_buf_RH, fvec);
  ...
  // --- Angle-based trapezoidal torque ---
  if (s_assist_active_R) {
    tau_R = ComputeAngleTrapTorque(
        &s_assist_elapsed_R_ms, &s_fall_elapsed_R_ms,
        &s_assist_phase_R, &s_assist_active_R, &Rstop_assist,
  ...
  // Keep FVec buffers flowing (not used for torque, but prevents stale state)
  (void)FVecDecoder_StepSingle(s_fvec_buf_RH);
  (void)FVecDecoder_StepSingle(s_fvec_buf_LH);
...
/*
 * Angle-based trapezoidal assist torque.
 *
 *  tau_max ┌──────────────────┐
 *         / 30ms    plateau    \ 30ms
 *        /                      \
 *   0 ──┘                        └── 0
 *     trigger            thigh >= target_angle
 *
 * Returns computed torque. Updates phase/elapsed/active in-place.
 */
static float ComputeAngleTrapTorque(
    float *elapsed_ms, float *fall_elapsed_ms,
    AssistPhase_t *phase, bool *active, bool *stop_assist,
  ...
  switch (*phase) {
  case PHASE_RISE:
    tau = tau_max * (*elapsed_ms / TRAP_RISE_MS);
    if (tau > tau_max)
      tau = tau_max;
    *elapsed_ms += CTRL_DT_MS;
    if (*elapsed_ms >= TRAP_RISE_MS) {
      *phase = PHASE_PLATEAU;
    }
    break;
  ...
  case PHASE_FALL:
    tau = tau_max * (1.0f - *fall_elapsed_ms / TRAP_FALL_MS);
    if (tau < 0.0f)
      tau = 0.0f;
    *fall_elapsed_ms += CTRL_DT_MS;
    *elapsed_ms += CTRL_DT_MS;
...
static inline void HC_UpdateDegThresh_OnSTS(float swing_deg_meas) {
  ...
  // Spec: threshold = 0.6 * (swing-leg angle at STS-classified HC), initial 10
  // deg.
  ...
  s_hc_deg_thresh =
      (1.0f - DEG_ALPHA) * s_hc_deg_thresh + DEG_ALPHA * deg_target;
...
static inline void UpPeak_UpdateThresh_OnSTS(float swing_deg_meas) {
  ...
  // Spec: s_thres_up = 0.8 * (swing-leg angle at STS-classified HC), initial 10
  // deg.
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (used < kk) {
      ...
      while (j > 0 && best_d2[j] < best_d2[j - 1]) {
        float tmpd = best_d2[j - 1];
        best_d2[j - 1] = best_d2[j];
        best_d2[j] = tmpd;
        uint8_t tmpl = best_lab[j - 1];
        best_lab[j - 1] = best_lab[j];
        best_lab[j] = tmpl;
        --j;
      }
...
static int update_standing_moving_flag_w_ud(float wR_metric, float wL_metric) {
  ...
  const float V_OFF_THRESH = 7.0f; // lowered from 10.0: eliminates false
                                   // MOVING->STANDING during slow walking
...
static void GaitModeRecognition_DetectEvents(const GaitFeatures_t *feat,
  ...
  if (res_out)
    res_out->is_moving = is_moving;

  if (!s_prev_is_moving && is_moving && feat) {
    // 0->1 순간: 현재 swing 중인 쪽을 선택
    const bool is_right_swing =
        (s_Rdeg[2] - s_Rdeg[1] >= s_Ldeg[2] - s_Ldeg[1]);
  ...
  if (s_R_time_afterup_ms >= s_t_gap_R_ms)
    s_R_time_upcond = true;
  if (s_L_time_afterup_ms >= s_t_gap_L_ms)
    s_L_time_upcond = true;
  if (s_HC_time_after_R_ms >= s_t_gap_R_ms)
    s_HC_time_upcond_R = true;
  if (s_HC_time_after_L_ms >= s_t_gap_L_ms)
    s_HC_time_upcond_L = true;
  ...
  if ((s_Rdeg[2] - s_Ldeg[2]) * (s_Rdeg[1] - s_Ldeg[1]) <= 0.0f) {
    ...
    const bool is_right_swing =
        (s_Rdeg[2] - s_Rdeg[1] >= s_Ldeg[2] - s_Ldeg[1]);
    const bool hc_time_ok =
        is_right_swing ? s_HC_time_upcond_R : s_HC_time_upcond_L;
    ...
    if (is_right_swing) {
      ...
      stance_angle_ok = (!s_R_stance_angle_sampled) ||
                        (stance_angle_sample >= s_R_stance_angle_gate);
    } else {
      stance_angle_sample = s_L_prev_stance_angle_at_dpeak;
      stance_angle_ok = (!s_L_stance_angle_sampled) ||
                        (stance_angle_sample >= s_L_stance_angle_gate);
    }
    ...
    if (!hc_time_ok || !swing_deg_ok || !stance_angle_ok) {
      // Not enough time elapsed (per swing side) or swing angle too small;
      // ignore this HC candidate.
    } else {
      if (is_right_swing) {
      ...
      if (is_right_swing) {
        ...
        if (s_adaptive_assist_enabled && s_gait_mode == RSTS) {
          // STS 분류 + adaptive ON → 목표각을 15도로 변경 (30ms fall)
          s_target_angle_R = TARGET_ANGLE_STS;
          if (s_Rdeg[2] >= TARGET_ANGLE_STS &&
              s_assist_phase_R == PHASE_PLATEAU) {
        ...
        if (s_adaptive_assist_enabled && s_gait_mode == LSTS) {
          // STS 분류 + adaptive ON → 목표각을 15도로 변경 (30ms fall)
          s_target_angle_L = TARGET_ANGLE_STS;
          if (s_Ldeg[2] >= TARGET_ANGLE_STS &&
              s_assist_phase_L == PHASE_PLATEAU) {
  ...
  if ((s_Rdeg[2] - s_Rdeg[1]) * (s_Rdeg[1] - s_Rdeg[0]) < s_var &&
      (s_Rdeg[2] - s_Rdeg[1]) < 0.0f && s_Rdeg[2] >= s_thres_up &&
      s_R_time_upcond) {
    ...
    // NOTE: this is a gate time used for event de-bounce, not the normalization
    // swing period.
    const float GAP_MIN_MS = 150.0f;
    const float GAP_MAX_MS = 1200.0f;
    const float GAP_ALPHA = 0.2f;
    const float meas_clamped =
        clampf_ud(measured_swing_ms, GAP_MIN_MS, GAP_MAX_MS);
    const float prev_gap_R = s_t_gap_R_ms;
    float updated_gap_R =
        (1.0f - GAP_ALPHA) * prev_gap_R + GAP_ALPHA * meas_clamped;
    if (prev_gap_R > 0.0f) {
    ...
    if (s_last_HC_class_is_valid && !s_last_HC_is_STS) {
      ...
      s_R_stance_angle_gate =
          (1.0f - ALPHA) * s_R_stance_angle_gate + ALPHA * target_thresh;
  ...
  if ((s_Ldeg[2] - s_Ldeg[1]) * (s_Ldeg[1] - s_Ldeg[0]) < s_var &&
      (s_Ldeg[2] - s_Ldeg[1]) < 0.0f && s_Ldeg[2] >= s_thres_up &&
      s_L_time_upcond) {
    ...
    float updated_gap_L =
        (1.0f - GAP_ALPHA) * prev_gap_L + GAP_ALPHA * meas_clamped;
    if (prev_gap_L > 0.0f) {
    ...
    if (s_last_HC_class_is_valid && !s_last_HC_is_STS) {
      ...
      const float target_thresh = clampf_ud(0.3f * raw_peak, 5.0f, 120.0f);
      const float ALPHA = 0.2f;
      s_L_stance_angle_gate =
          (1.0f - ALPHA) * s_L_stance_angle_gate + ALPHA * target_thresh;
  ...
  if ((s_Rdeg[2] - s_Rdeg[1]) * (s_Rdeg[1] - s_Rdeg[0]) <= s_var &&
      (s_Rdeg[2] - s_Rdeg[1]) >= 0.0f && s_Rdeg[2] <= s_R_lowpeak_val + 0.5f) {
    R_count_dpeak++;
    if (is_moving) {
  ...
  if ((s_Ldeg[2] - s_Ldeg[1]) * (s_Ldeg[1] - s_Ldeg[0]) <= s_var &&
      (s_Ldeg[2] - s_Ldeg[1]) >= 0.0f && s_Ldeg[2] <= s_L_lowpeak_val + 0.5f) {
    L_count_dpeak++;
    if (is_moving) {
  ...
  if (res_out) {
    ...
    // Rising-after-lower should only fire once when the flag was armed and then
    // the rise condition is met.
    const bool r_rise_now =
        (Rflag_assist && (s_Rdeg[2] > s_R_lowpeak_val + 3.0f));
    const bool l_rise_now =
        (Lflag_assist && (s_Ldeg[2] > s_L_lowpeak_val + 3.0f));
...
static void adaptive_assist(const GaitRecognitionResult_t *rec,
  ...
  if (rec != NULL) {
    if (rec->r_uprise_after_lower && !s_assist_active_R &&
      ...
      // Angle-based trapezoidal assist setup
      s_assist_active_R = (tau_max_setting > 0);
      s_assist_tau_max_R = (float)tau_max_setting * 1.0f;
      s_assist_elapsed_R_ms = 0.0f;
      s_assist_phase_R = PHASE_RISE;
...
static void ResetUserState(void) {
  ...
  // Angle-based trapezoidal assist reset
  s_assist_active_R = false;
  s_assist_active_L = false;
  s_assist_tau_max_R = 0.0f;
  s_assist_tau_max_L = 0.0f;
  s_assist_elapsed_R_ms = 0.0f;
  s_assist_elapsed_L_ms = 0.0f;
  s_target_angle_R = TARGET_ANGLE_DEFAULT;
  s_target_angle_L = TARGET_ANGLE_DEFAULT;
  ...
  s_adaptive_assist_enabled = false;
  s_btn3_press_timer = 0;
  s_tau_out_R_old1 = 0.0f;
  s_tau_out_R_old2 = 0.0f;
  s_tau_out_L_old1 = 0.0f;
  s_tau_out_L_old2 = 0.0f;
...
static float FVecDecoder_StepSingle(FVecSlot_t buf[]) {
  ...
  for (int i = 0; i < F_VECTOR_BUFF_SIZE; i++) {
    ...
    float tau_raw = p->b1 * s->tau_old1 + p->b2 * s->tau_old2 + p->a0 * s->u +
                    p->a1 * s->u_old1 + p->a2 * s->u_old2;
...
static void TriggerManuscriptFVecSingle(FVecSlot_t buf[], const float fvec[4]) {
  ...
  // Deterministic +1 control tick offset so the overlay starts on the next
  // control tick.
