...
DOPI_SDOUnit_t DOPI_CreateSDOUnit_payload(DOPI_DevObj_t* t_obj, uint8_t t_taskID, uint16_t t_SDOID, DOP_SDOStatus_t t_SDOStatus, const void* payload, uint8_t payloadSize);
...
DOP_Header_t GetHeader(uint8_t* byte_arr);
DOP_SDOArgs_t Bytes2SDOreq(uint8_t* byte_arr, uint16_t *byte_len);
