...
#else
osStatus_t _debug_usbh_process_os_status;
...
USBH_StatusTypeDef USBH_DeInit(USBH_HandleTypeDef *phost)
{
  ...
  _debug_usbh_process_os_status = 0;
...
uint32_t _debug_usbh_process_stage;
USBH_StatusTypeDef _debug_usbh_process_status;
/**
  * @brief  USBH_Process
...
USBH_StatusTypeDef USBH_Process(USBH_HandleTypeDef *phost)
{
  ...
  switch (phost->gState)
  {
    ...
    case HOST_ENUMERATION:
      ...
      _debug_usbh_process_status = status;
      if (status == USBH_OK)
      {
...
#else
uint32_t _debug_usbh_process_os_counter = 0;
...
static void USBH_Process_OS(void *argument)
{
  ...
  {    
    ...
    _debug_usbh_process_os_status = status;
    if (status == osOK)
    {
      USBH_Process((USBH_HandleTypeDef *)argument);
    }
    _debug_usbh_process_os_counter++;
  }
}
#endif /* (osCMSIS < 0x20000U) */
